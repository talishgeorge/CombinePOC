...
class Services {
    ...
    func getWeatherData(city: String?, success: @escaping SuccessCompletionBlock, failure: @escaping FailureBlock) {
        var baseOpenWeatherURL = ""
        if let cityName = city {
            baseOpenWeatherURL = "https://api.openweathermap.org/data/2.5/weather?q=\(cityName)&Appid=315a5a4dae4ad2b0554677c7fdfdada1"
        }
...
//            let kitError = APIError(code: error, message: response.data, description: code)
//            let customError = NSError(domain: "", code: code, userInfo: [ NSLocalizedDescriptionKey: kitError.description ?? ""]) as Error
            let errorHandler = CommonErrorsUtility(jsonData: response.data, withOption: .mutableContainers)
